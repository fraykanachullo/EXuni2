...
class VerifyEmail { 
    ...
    public function check($email) {
        ...
        // Verificar si $this->stream es un recurso válido antes de cerrarlo
        ...
        switch ($code) {
            case '250':
                // Email address was accepted
                $result->isValid = true;
                $result->details = 'Email address is valid and accepted.';
                break;
            case '450':
            case '451':
            case '452':
                // Temporary issues, email may still be valid
                $result->details = 'Temporary issue on the server. Try again later.';
                break;
            case '550':
                // Email address was rejected
                $result->details = 'Email address is not valid.';
                break;
            default:
                // Other cases
                $result->details = 'Unexpected response from the server.';
                break;
        }
    ...
    protected function _streamQuery($query) {
        ...
        if ($this->stream && is_resource($this->stream)) {
            ...
            $this->set_error('Socket is not a valid resource.');
            $this->edebug($this->ErrorInfo);
            if ($this->exceptions) {
                throw new verifyEmailException($this->ErrorInfo);
            }
